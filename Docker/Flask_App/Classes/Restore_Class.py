...
class Backup_Class(Task_Handler_Class):
    ...
    def task(self, script_path:str, script_upload_path:str, script_run_command:str, script_parameters:str, ssh_username:str, ssh_password:str, ip_addresses:list[str]) -> int:
        self.logger.info(f"Restoring {ip_addresses} backup ...")
        ...
        if len(failed_ip_addresses) > 0:
            self.logger.info(f"Restore Failed for IP Addresses: {failed_ip_addresses}")
        self.logger.info(f"Restore Finished for IP Addresses: {[ip for ip in ip_addresses if ip not in failed_ip_addresses]}")
